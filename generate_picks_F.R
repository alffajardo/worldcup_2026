...
picks_id <- drive_find(type = "spreadsheet",pattern = "WC2026: KO Stage F",
                       n_max = 1)$id
...
write.table(bets,"F_picks.csv",sep=",",
            quote = F,
            row.names =F )


write.csv(scores_prediction,file = "F_predicted_scores.csv",
          quote = F,
          row.names =F )
...
ggsave("picks_F.png",plot = plot1,units = "px",width = 1200,height = 1200,
...
ggsave("predicted_scores_F.png",plot = plot2,units = "px",width = 1200,height = 1000,
       path = "media",dpi = 200,bg="white")
